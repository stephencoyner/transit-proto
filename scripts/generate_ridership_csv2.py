...
def generate_stop_ridership(
    ...
    running_load = 0
    final_max_load = 0
    total_load = 0  # For calculating avg_load
    ...
    # Calculate average load across all stops
    final_avg_load = round(total_load / n_stops, 2) if n_stops > 0 else 0

    return result, final_max_load, final_avg_load
...
def load_sample_trips(path: str) -> Dict[str, List[Dict]]:
    """
    Load sample_trips CSV to get stop sequences per route.
    ...
    for route_id in route_stops:
        route_stops[route_id].sort(key=lambda x: x['stop_sequence'])

    print(f"Loaded stop sequences for {len(route_stops)} routes")
...
def generate_ridership(
    ...
    # Column order per spec: date, trip_id, route_id, direction_id, start_time, time_period, day_of_week, ...
    trip_writer.writerow([
        'date', 'trip_id', 'route_id', 'direction_id',
        'start_time', 'time_period', 'day_of_week', 'total_boardings', 'total_alightings', 'avg_load', 'max_load'
    ])
    ...
    def open_new_stop_chunk():
        ...
        # Column order per spec: date, trip_id, route_id, stop_id, stop_sequence, boardings, alightings, load_after, day_of_week, direction_id, time_period
    ...
    while current_date <= GENERATE_END:
        ...
        system_stats = {
            ...
            "total_boardings": 0,
            "total_alightings": 0,
            "total_avg_load": 0,  # Sum of all trip avg_loads
            "load_count": 0,
            "max_load": 0
        }
        ...
        for route_id, trips in route_trips.items():
            ...
            route_stats = {
                ...
                "total_boardings": 0,
                "total_alightings": 0,
                "total_avg_load": 0,  # Sum of trip avg_loads (for calculating route avg_load)
                "load_count": 0,
                "max_load": 0
            }
            ...
            for trip in trips:
                ...
                stop_data, max_load, avg_load = generate_stop_ridership(
                ...
                # Write trip record (column order per spec)
                trip_writer.writerow([
                ...
                route_stats["total_avg_load"] += avg_load  # Sum of trip avg_loads
                route_stats["load_count"] += 1
                route_stats["max_load"] = max(route_stats["max_load"], max_load)

            # Route daily summary - avg_load is now average of trip avg_loads
            route_avg_load = route_stats["total_avg_load"] / route_stats["load_count"] if route_stats["load_count"] > 0 else 0
            route_summaries.append([
                date_str, day_of_week, route_id, route_stats["trip_count"],
                route_stats["total_boardings"], route_stats["total_alightings"],
                round(route_avg_load, 2), route_stats["max_load"]
            ])
            ...
            system_stats["total_avg_load"] += route_stats["total_avg_load"]  # Sum of all trip avg_loads
            system_stats["load_count"] += route_stats["load_count"]
            system_stats["max_load"] = max(system_stats["max_load"], route_stats["max_load"])

        # System daily summary - avg_load is average of all trip avg_loads
        sys_avg_load = system_stats["total_avg_load"] / system_stats["load_count"] if system_stats["load_count"] > 0 else 0
...
def main():
    ...
    parser.add_argument("--sample-trips", required=True, help="Path to sample_trips.csv")
    ...
    route_stop_sequences = load_sample_trips(args.sample_trips)
